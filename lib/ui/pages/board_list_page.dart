...
class _BoardListPageState extends State<BoardListPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final categories = categoriesRaw.map((e) => BoardCategory.fromMap(e)).toList();
      ...
      for (final category in categories) {
        ...
        for (final board in boards) {
          ...
          final paths = imagesRaw.map((e) => e['filePath'] as String).take(5).toList();
  ...
  Future<void> _createCategoryDialog() async {
    ...
    await showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        content: TextField(controller: nameController, decoration: const InputDecoration(hintText: "Category Name"), autofocus: true),
        actions: [
  ...
  Future<void> _createBoardDialog({BoardCategory? preSelectedCategory}) async {
    ...
    BoardCategory? selectedCategory = preSelectedCategory ?? (categories.isNotEmpty ? categories.first : null);
    if (!mounted) return;
    await showDialog(
      context: context,
      builder: (c) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              TextField(controller: nameController, decoration: const InputDecoration(hintText: "Board Name"), autofocus: true),
            ...
            ElevatedButton(
              onPressed: () async {
                if (nameController.text.isNotEmpty && selectedCategory != null) {
                  await _boardRepo.createBoard(nameController.text, categoryId: selectedCategory!.id);
                  if (c.mounted) Navigator.pop(c);
                  _loadData(); 
                }
              },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          "Cookie point", 
          style: TextStyle(color: Colors.black, fontWeight: FontWeight.w600, fontSize: 24, fontFamily: 'GeneralSans'),
        ),
        actions: [
          IconButton(icon: const Icon(Icons.hexagon_outlined, color: Colors.black), onPressed: () {}),
        ],
      ),
      body: _isLoading
          ...
          : Column(
              children: [
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Row(
                          children: const [
                            Text("Global", style: TextStyle(fontWeight: FontWeight.w500, fontFamily: 'GeneralSans', color: Colors.black87)),
                            SizedBox(width: 8),
                      ...
                      const Spacer(),
                      const Icon(Icons.splitscreen_outlined, size: 24, color: Colors.black87), 
                      const SizedBox(width: 12),
                      const Icon(Icons.grid_view, size: 24, color: Colors.black87),
                    ],
                  ),
                ),
                ...
                // Main Content
                Expanded(
                  child: _categorizedBoards.isEmpty
                      ? Center(child: ElevatedButton(onPressed: _createCategoryDialog, child: const Text("Create Your First Category")))
                      : ListView.builder(
                          padding: const EdgeInsets.only(bottom: 40),
                          itemCount: _categorizedBoards.length,
                          itemBuilder: (context, index) {
                            final category = _categorizedBoards.keys.elementAt(index);
                                ...
                                if (boards.isEmpty)
                                  Padding(
                                    ...
                                    child: GestureDetector(
                                      onTap: () => _createBoardDialog(preSelectedCategory: category),
                                      child: const Text("+ Add Board", style: TextStyle(color: Colors.blue, fontWeight: FontWeight.w500)),
                                    ),
                                  )
                                else
                                  ...boards.map((board) => _buildBoardPreviewCard(board, category.name)),
  ...
  Widget _buildBoardPreviewCard(Board board, String categoryName) {
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(context, MaterialPageRoute(builder: (_) => BoardDetailPage(board: board))).then((_) => _loadData());
      },
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Text(board.name, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w500, fontFamily: 'GeneralSans')),
                  const Spacer(),
                  GestureDetector(
                    onTap: () {
                       Navigator.push(context, MaterialPageRoute(builder: (_) => BoardDetailPage(board: board))).then((_) => _loadData());
            ...
            SizedBox(
              ...
              child: ListView.separated(
                ...
                separatorBuilder: (_, __) => Container(
                  width: 1,
                  color: const Color.fromARGB(255, 169, 169, 169), // Visible grey divider
                ),
                itemBuilder: (context, index) {
