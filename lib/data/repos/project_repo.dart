...
class ProjectRepo {
  ...
  Future<void> updateStylesheet(int id, String jsonStylesheet) async {
    final db = await AppDatabase.db;
    await db.update('projects', {'global_stylesheet': jsonStylesheet}, where: 'id = ?', whereArgs: [id]);
  }

  Future<void> touchProject(int id) async {
    final db = await AppDatabase.db;
    await db.update('projects', {'last_accessed_at': DateTime.now().toIso8601String()}, where: 'id = ?', whereArgs: [id]);
  }

  Future<List<int>> getAllSubEventIds(int projectId) async {
    final db = await AppDatabase.db;
    final res = await db.query('projects', columns: ['id'], where: 'parent_id = ?', whereArgs: [projectId]);
